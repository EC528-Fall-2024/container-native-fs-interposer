...
static const struct fuse_opt fuse_attr[] = {                //option description
	{ "writeback",
...
/*
 *  this is where all the function libfuse kernel module is gonna look for 
 *  */
...
/* opts.foreground will be hard coded to set to true 
 * opts.max_idle_threads will be hard coded to set to 1
 * */
int main(int argc, char *argv[])
{
    struct fuse_args args = FUSE_ARGS_INIT(argc, argv);             //args are parsed
    struct fuse_session *se;                                        //handler
    struct fuse_cmdline_opts opts;
    struct fuse_loop_config config;
    struct lo_data fuse_root_data;
    int ret = -1;

    //pre initializing fuse_root_data
    pthread_mutex_init(&fuse_root_data.mutex, NULL);         //not gonna support multiple write for now
    fuse_root_data.source = NULL;
    fuse_root_data.root.next = fuse_root_data.root.prev = &fuse_root_data.root;
    fuse_root_data.root.fd = -1;
    fuse_root_data.cache = CACHE_NORMAL;

    config.max_idle_threads = 1;

    if(fuse_parse_cmdline(&args, &opts) != 0)
        return -1;
    if( fuse_opt_parse(&args, &fuse_root_data, fuse_attr, NULL) == -1)
        return -1;
    ...
    //initializing fuse_root_data after parsed cmdline
    fuse_root_data.debug = 0;
    fuse_root_data.root.refcount = 2;
    if(fuse_root_data.source) {
        printf("mirroring source dir : %s \n", fuse_root_data.source);
        struct stat stat;
        int res;
        res = lstat(fuse_root_data.source, &stat);
        if(res == -1) {
            printf("cannot lstat the source directory \n");
            exit(1);
        }
        if(!S_ISDIR(stat.st_mode)) {
            printf("source provided is not a directory \n");
            exit(1);
        }
    } else {
        fuse_root_data.source = strdup("/");
        assert(strcmp(fuse_root_data.source, "/") == 0);
    }
    if(!fuse_root_data.timeout_set) {
        switch(fuse_root_data.cache) {
            case CACHE_NEVER:
                fuse_root_data.timeout = 0.0;
                break;

            case CACHE_NORMAL:
                fuse_root_data.timeout = 1.0;
                break;

            case CACHE_ALWAYS:
                fuse_root_data.timeout = 86400.0;
                break;
        }
    } else if(fuse_root_data.timeout < 0.0) {
        printf("timeout cannot be less than 0 \n");
        exit(1);
    }

    fuse_root_data.root.fd = open(fuse_root_data.source, O_PATH);
    if(fuse_root_data.root.fd == -1) {
        printf("cannot open the source: %s \n", fuse_root_data.source);
        exit(1);
    }

    
    
    opts.foreground = 1;                //hard coded to be true for testing purpose
    opts.max_idle_threads = 1;          //hard coded

    se = fuse_session_new(&args, &operations, sizeof(operations), &fuse_root_data); 
    if( se == NULL ) {
    ...
    ret = fuse_session_loop_mt(se, &config);               //block untill ctl+c or fusemount -u (unmount).  
