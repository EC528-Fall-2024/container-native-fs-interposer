...
extern struct fuse_operations* new_nop_layer(struct fuse_operations*);
...
{
    struct fuse_operations* oper_ptr = new_nop_layer(&xmp_oper);
	enum { MAX_ARGS = 10 };
	int i,new_argc;
	...
	return fuse_main(new_argc, new_argv, oper_ptr, NULL);
}