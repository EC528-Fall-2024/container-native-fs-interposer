use crate::fuse::{
    ...
    timespec, uid_t, fuse_lowlevel_ops
};
use std::{
...
static mut PT: MaybeUninit<passthrough::PassthroughFs> = MaybeUninit::uninit();

unsafe extern "C" fn lo_init(userdata: *mut c_void, userconn: *mut fuse_conn_info) {
...
#[no_mangle]
pub unsafe extern "C" fn ll_layer() { // -> *const fuse_lowlevel_ops {
    
    let fs_cfg = passthrough::Config {
        root_dir: "/tmp/test".to_string(),
        ..Default::default()
    };
    let fs = passthrough::PassthroughFs::new(fs_cfg).unwrap();
    PT.write(fs);
    
    //Box::into_raw(Box::new(fuse_lowlevel_ops{
    //    init: lo_init
    //}))
}
